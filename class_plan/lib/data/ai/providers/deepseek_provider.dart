...
class DeepSeekProvider extends AiProvider {
  ...
  Future<List<StructuredCourse>> parse(RawScheduleData raw) async {
    throw UnimplementedError('请在设置中配置 DeepSeek API Key');
  }
  ...
  Future<String> callApi(String text, String apiKey) async {
    ...
    if (response.statusCode != 200) {
      throw Exception('DeepSeek API 错误：${response.statusCode} ${response.body}');
    }
    ...
    if (firstChoice is! Map) throw Exception('DeepSeek 返回格式错误');
    final message = firstChoice['message'] as Map?;
    if (message == null) throw Exception('DeepSeek 返回格式错误');
